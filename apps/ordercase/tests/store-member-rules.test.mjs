...
test("店舗マスターは店舗指名・店舗NGと理由を編集できる", () => {
  ...
  assert.match(pageSource, /preferred_note:editPreferredNote\.value\.trim\(\)/);
  assert.match(pageSource, /ng_note:editNgNote\.value\.trim\(\)/);
});

test("指名・NGのIDは重複を除去し不正値を拒否する", () => {
  ...
  assert.match(serviceSource, /同じメンバーを指名とNGの両方には登録できません/);
});
